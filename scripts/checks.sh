...
# rust-toolchain.toml), `fmt --check`, `tsc --noEmit`, and commitlint on the
# commit range. Running the real gates locally is the only reliable way to
# avoid the "passed here, failed in CI" round trip.
#
# Usage:
...
step "Gate feedback repro"
DEMETEO_REPO_PATH="$ROOT" node tests/repro/gate-feedback-rerender.mjs
