...
- Two fences, neither ever widened: Demeteo's own artifact-scope chmod/ACL in `adapters/worktree/git_ops/scope.rs`, which every harness gets, and the harness's own, which varies by harness *and* by class of access — `PathContainment` (`domain/models/sandbox.rs`) is the only statement of the second, and no surface may promise more than it carries
...
| Discovery sessions & dependency-gated tickets (specified, not built) | [docs/PRD_DISCOVERY.md](docs/PRD_DISCOVERY.md) |
