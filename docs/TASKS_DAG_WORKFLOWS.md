...
- **Goal:** Ship what is genuinely topological in the PRD §7 shapes: Standard = `research ∥ baseline-harness` fan-in to `tickets`, and `validate ∥ critic` fan-in to `gate-ship`; Refactor = `regression ∥ api-drift-review` fan-in to `gate-diff`. The baseline-node conversions themselves are P4.2a's; this task only rewires the edges. The other starters remain chains; each test-gated starter now begins with the same baseline measurement.
