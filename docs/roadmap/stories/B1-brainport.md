...
- `MrPublisher` port (`ports/mr_publisher.rs:19`) is where PR-description generation plugs in; adapter is `adapters/mr_publisher.rs` (876 lines, GitHub + PublishOptions taking a `description` field already, since it's idempotent on `mr_url`).
...
**Tasks:**
- [ ] In the `MrPublisher` flow (`ports/mr_publisher.rs`, `adapters/mr_publisher.rs`), call `BrainPort` with the feature's diff + step summaries to generate `PublishOptions`'s description field before calling `publish_mr`.
