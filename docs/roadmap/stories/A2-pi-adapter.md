...
> **Antigravity decision record (already made, do not re-litigate):** `agy`'s headless mode is query-only by default, requires `--dangerously-skip-permissions` for tool execution, has no documented structured output, and reportedly drops `--print` output under non-TTY stdout (our exact invocation path). Given all that, the antigravity adapter was **removed entirely** (2026-07-10, coding-agent consistency initiative) and the monthly `agy` re-probe (formerly A3.2 / A4.4) was dropped with it; pi is what covers Google/Gemini models. See [market research §1](../01-market-research.md) for the historical churn lesson. If a native Antigravity adapter is ever wanted again, scope it fresh against the current `AgentRuntime` + `AgentCapabilities` contract.
