...
- [ ] F38: `WorkflowEditor.tsx` has no dirty-state guard — Back arrow (`:204`), Escape/`Cmd+W`, and mouse-back all silently discard unsaved steps/prompt-templates/schedule edits. Add a dirty-state check that warns before discarding (prompt templates especially can't be recreated from memory).
...
- [ ] F44: `syncFeature`/`resolveSyncConflicts` accept `revalidateStepExecutionId` to replay validation after a merge; `FeatureDetail.tsx:552,863` always passes `null`, so the "resolved" banner tells the user to manually confirm the build still works — a step the backend was designed to automate. Wire the actual step id through.
...
**Tasks:**
- [ ] F14 (**reduced in scope — the `'remote-inbox'` half is resolved by deletion, July 2026**): the reported symptom was re-clicking "Remote runs" pushing a duplicate back entry, because `NavigationContext.tsx:20-48`'s `shallowEqualView` had no case for `'remote-inbox'` (falls to `default: return false`). That view no longer exists — the Runs tab was removed (see `docs/REMOTE_EXECUTION_PLAN.md` M6.2 amendment) — so there is no missing case left to add. **What remains:** `shallowEqualView` still returns `false` by default for any kind it doesn't enumerate, so the next view kind added silently inherits the same duplicate-back-entry bug. Add a sane `default` that compares by view kind.
...
- [ ] Along the way, dedupe the smaller repeated helpers noted in the same finding: `formatTokens` (`FeatureDetail.tsx:109` vs `lib/utils.ts:1`) and `fuzzyMatch` (`ProjectRail.tsx:6` vs `CommandPalette.tsx:19`) — small, low-risk cleanup that's natural to bundle with this consolidation pass. (The relative-time helper was a third case; its `RemoteRunInbox.tsx:88` copy died with the Runs tab, so only `NotificationBell.tsx:257` is left and there is nothing to dedupe.)
