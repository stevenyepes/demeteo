...
**Epic acceptance:** all working adapters covered; one simulated breaking change caught by CI in a drill.

**Why this matters now, not later:** the Gemini→Antigravity churn (Google killed Gemini CLI and replaced it inside one release cycle, see [market research §1](../01-market-research.md)) is the proof case — and the reason the `antigravity` adapter was ultimately removed rather than maintained. This must land *before* Epic C1's kanban board multiplies concurrent agent runs (see roadmap's "Deliberate sequencing couplings").

**Grounding facts (verified in repo, 2026-07-10):**
- The adapters to cover: `claude_code`, `hermes`, `opencode` (existing, `crates/demeteo-core/src/adapters/agent/{name}/mod.rs`) plus whichever of Codex/pi land first from Epics A1/A2. (The `antigravity` adapter was removed — see the consistency initiative; do not backfill a fixture for it.) Each exposes a `runtime()` constructor consumed by `AgentRegistry` (`adapters/agent/registry.rs:13`), which is wired by hand in `crates/demeteo-core/src/composition/mod.rs`. Each also declares an `AgentCapabilities` descriptor (`ports/agent_runtime.rs`).
...
- [ ] Backfill fixtures for the three already-shipped adapters (`claude_code`, `hermes`, `opencode`) — these don't exist yet and are needed to satisfy "all working adapters covered."
