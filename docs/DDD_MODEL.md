...
- **Value Objects:** `AgentKind` (`opencode` | `hermes` | `claude-code` | `codex` | `pi`) — a real enum (`domain/models/agent_config.rs`) whose `as_str` equals the runtime `kind()` key; `AgentCapabilities` (`display_label`, `model_listing`, `default_model`) declared once per runtime so no downstream site matches on the kind string; `AgentConfig`, `AgentContext`, `AgentEvent`, `StepCapability`, `PermissionProfile`, `WriteScope`, `Access`, `Usage`, `StopReason`
