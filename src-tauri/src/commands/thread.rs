...
#[tauri::command]
pub fn get_thread_sessions(ctx: State<'_, AppContext>, machine_id: String) -> Result<Vec<ThreadSession>, String> {
    ctx.threads.get_thread_sessions(&crate::domain::ids::MachineId::from(machine_id))
}
...
pub fn add_thread_session(
    ...
    if thread.mode == "worktree" {
        if let (Some(repo_path), Some(branch), Some(sandbox_path)) = (&thread.repo_path, &thread.branch, &thread.sandbox_path) {
            ctx.exec.setup_worktree(thread.machine_id.as_str(), repo_path, branch, sandbox_path)?;
        } else {
...
pub fn update_thread_status(
    ...
    ctx.threads.update_thread(&ThreadId::from(id), &ThreadPatch { status: Some(status), ..Default::default() })
}
