 ...
 * share. These prove the two inputs fold into one shape — the authoritative
 * `step_executions` snapshot for status/cost/duration, and the `run_events`
 * stream for what a step row can't carry (the failure class, the spawn
 * evidence) — and that events for other runs are ignored.
 *
 * Spawn evidence comes out keyed by `step_execution_id`, never projected onto
 * a node: `useRunGraph` owns that join because it is also where a detached run
 * substitutes its own evidence.
 */
...
describe('useRunEvents', () => {
  it('backfills from zero and merges a newer live event before history resolves', async () => {
    ...
    expect(result.current.statusByNode.implement.stepExecutionId).toBe('se-1');
    expect(result.current.assignments['se-1']).toEqual({
  ...
  it('retains assignments after their event leaves the bounded feed', async () => {
    ...
    expect(result.current.assignments['se-1']).toMatchObject({
      agentKind: 'hermes',
      effort: null,
    });
  });

  it('keeps evidence for every execution while the overlay picks the newest', async () => {
    listRunEventsSince.mockResolvedValue([
    ...
    // The node keeps one execution; the earlier attempt keeps its own evidence,
    // which is what lets the timeline show what each attempt actually ran.
    await waitFor(() =>
      expect(result.current.assignments['se-new']).toMatchObject({
        agentKind: 'claude-code',
        effort: 'medium',
      }),
    );
    expect(result.current.statusByNode.implement.stepExecutionId).toBe('se-new');
    expect(result.current.assignments['se-old']).toMatchObject({
      agentKind: 'opencode',
      effort: 'max',
    });
    expect(result.current.assignments['se-other']).toBeUndefined();
  });
  ...
  it('isolates feature state and ignores a late history response from the previous feature', async () => {
    ...
    expect(result.current.statusByNode.research.stepExecutionId).toBe('se-2');
    expect(result.current.assignments).toEqual({
      'se-2': { stepExecutionId: 'se-2', agentKind: 'hermes', effort: 'low', offset: 2 },
    });
  });
