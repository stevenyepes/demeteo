...
describe('useTerminalPanel — startup replay buffer', () => {
  it('returns null when the seed channel has not captured any bytes', async () => {
    const h = mountHarness();
    await act(async () => {
      await h.panel.open({ machineId: 'local', machineLabel: 'local' });
    });
    const tabId = h.panel.state.tabs[0]!.tabId;
    expect(h.panel.consumeStartupReplay(tabId)).toBeNull();
  });

  it('returns null for a tabId that never existed', () => {
    const h = mountHarness();
    expect(h.panel.consumeStartupReplay('nope')).toBeNull();
  });

  it('drops the buffer entry when the tab is closed before any consume', async () => {
    const h = mountHarness();
    let tabId = '';
    await act(async () => {
      tabId = await h.panel.open({ machineId: 'local', machineLabel: 'local' });
  ...
  it('captures seed-channel bytes and replays them exactly once', async () => {
    // Pull the latest ChannelStub instance the provider just constructed
    // for the seed channel. The mock in test/setup.ts stores every new
    // Channel on the global so the most-recent one is the seed.
    const ChannelMod = await import('@tauri-apps/api/core');
    // Track all channel instances created during the test so we can
    // pick the FIRST one as the seed.
    const instances: Array<{
      id: number;
      onmessage: ((chunk: Uint8Array | number[]) => void) | null;
    }> = [];
    const originalCtor = ChannelMod.Channel;
    const TrackingChannel: unknown = function (this: unknown) {
      const inst = new originalCtor();
      instances.push(inst as unknown as { id: number; onmessage: ((chunk: Uint8Array | number[]) => void) | null });
      return inst;
    };
    (TrackingChannel as { prototype: object }).prototype = (
      originalCtor as unknown as { prototype: object }
    ).prototype;
    (ChannelMod as { Channel: unknown }).Channel = TrackingChannel;
    const ChannelReset = ChannelMod.Channel;
    try {
      const h = mountHarness();
      await act(async () => {
        await h.panel.open({ machineId: 'local', machineLabel: 'local' });
      ...
      // First Channel instance = the seed. The second one (created by
      // the surface's `attach_terminal_session` later in another test
      // path) is irrelevant here.
      const seed = instances[0]!;
      expect(seed.onmessage).not.toBeNull();

      // Push two startup chunks through the seed channel — this is
      // what the backend drain thread does for every output byte.
      seed.onmessage!(new Uint8Array([104, 105])); // "hi"
      seed.onmessage!(new Uint8Array([33])); // "!"

      const tabId = h.panel.state.tabs[0]!.tabId;

      // First consume returns the concatenated replay buffer.
      const replay = h.panel.consumeStartupReplay(tabId);
      expect(replay).not.toBeNull();
      expect(Array.from(replay!)).toEqual([104, 105, 33]);

      // Second consume is a no-op (buffer drained, seed deactivated).
      expect(h.panel.consumeStartupReplay(tabId)).toBeNull();

      // Subsequent bytes are silently dropped — no new buffer entry,
      // no exception.
      seed.onmessage!(new Uint8Array([1, 2, 3]));
      expect(h.panel.consumeStartupReplay(tabId)).toBeNull();
    } finally {
      (ChannelMod as { Channel: unknown }).Channel = ChannelReset;
    }
  });
});
