...
fn attempt_numbers_are_dense_and_one_based() {
    let db = db();
    let id = sid("se-1");
    assert_eq!(attempt_open(&db, &id, 100).unwrap(), 1);
    attempt_close(
        &db,
        &id,
        1,
    ...
    .unwrap();
    assert_eq!(attempt_open(&db, &id, 200).unwrap(), 2);
    // A different step execution numbers independently.
    assert_eq!(attempt_open(&db, &sid("se-2"), 200).unwrap(), 1);
}
...
fn close_persists_the_attempts_own_outcome() {
    let db = db();
    let id = sid("se-1");
    let no = attempt_open(&db, &id, 100).unwrap();
    attempt_close(
        &db,
        &id,
        no,
...
fn reopening_interrupts_a_stale_running_attempt() {
    ...
    assert_eq!(attempt_open(&db, &id, 500).unwrap(), 2);
