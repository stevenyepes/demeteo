// The finalize agent's answer, and what is published when there was none.
// `super` = `domain::finalize::authored`.
...
/// passed. The warning is the only trace the reviewer gets.
#[test]
fn a_bypassed_hook_is_declared_in_the_body() {
    let a = Authored {
        commit_subject: "s".to_string(),
        commit_body: String::new(),
        pr_title: "t".to_string(),
