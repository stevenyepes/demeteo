...
impl HttpMrPublisher {
    async fn publish_mr_inner(
        ...
        // remote set-url`.
        let remote_user = if provider.kind.to_lowercase() == "github" {
            "x-access-token"
        } else {
            "oauth2"
        };
        let remote_url = format!("https://{}@{}/{}", remote_user, provider.host, repo_path);
        ...
        self.exec
            .write_file_bytes(machine_str, &askpass_path, askpass_script.as_bytes())
            .await
